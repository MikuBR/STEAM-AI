...
export default function Dashboard({ user }: { user: User }) {
  ...
  const getActiveApiKey = (): string | null => {
    return customApiKey || localStorage.getItem('custom_gemini_api_key');
  };
  ...
  const generateRecommendations = async () => {
    ...
    if (STATIC_MODE && !apiKey) {
      setGenerationError(
        'Modo estático: insira sua chave de API do Gemini nas configurações (engrenagem) para gerar recomendações. Sem um servidor backend, sua chave pessoal é necessária.'
      );
      setGenerating(false);
      return;
    }
    ...
    try {
      if (STATIC_MODE && apiKey) {
        const combinedLibrary = [...manualGames, ...ownedGames].slice(0, 50);
        const recs = await fetchRecommendations(
          apiKey,
          combinedLibrary,
          recentGames,
          { moreOf: prefsMoreOf, lessOf: prefsLessOf }
        );
  ...
  const handleSearchGames = async (e: React.FormEvent) => {
    ...
    try {
      if (STATIC_MODE) {
        const data = await searchSteamGames(searchQuery);
  ...
  const handleSearchManualGame = async (e: React.FormEvent) => {
    ...
    try {
      if (STATIC_MODE) {
        const data = await searchSteamGames(manualSearchQuery);
            ...
            >
              ...
              <div className="max-w-xl space-y-3">
                <p className="text-xs text-[#6D5C54] leading-relaxed font-medium">
                  ...
                  {STATIC_MODE && (
                    <span className="block mt-2 p-2 bg-amber-50 border border-amber-200 rounded-xl text-amber-800">
                      ⚡ Modo estático (GitHub Pages): insira sua chave Gemini pessoal nas configurações para gerar recomendações. 
                      A chave da cafeteria não está disponível sem o servidor backend.
                    </span>
                  )}
                </p>
          ...
          >
            ...
            {activeTab === 'library' && (
              <div className="space-y-6">
                {STATIC_MODE && (
                  <div className="p-5 bg-amber-50 border border-amber-200 rounded-3xl flex items-start gap-3 shadow-sm">
                    <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="text-sm font-bold text-amber-800">Modo estático — biblioteca Steam indisponível</h4>
                      <p className="text-xs text-amber-700 leading-relaxed mt-0.5">
                        A API da Steam não pode ser acessada diretamente do navegador por questões de CORS. 
                        Use a aba <span className="font-extrabold underline text-amber-800 cursor-pointer" onClick={() => setActiveTab('manual')}>Biblioteca Manual</span> 
                        {' '}para adicionar seus jogos manualmente. Seus dados persistem localmente no navegador.
                      </p>
                    </div>
                  </div>
                )}
